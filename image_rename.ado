...
program define imagerename
    ...
    confirm variable `idvar' `imagevar' `datevar' `consentvar'
    ...
    gen `clean_media' = "`mediadir'/" + substr(`imagevar', strpos(`imagevar', "\") + 1, .)
    replace `clean_media' = "`mediadir'/" + substr(`imagevar', strpos(`imagevar', "/") + 1, .) ///
        if strpos(`imagevar', "\") == 0 & strpos(`imagevar', "/") > 0
    replace `clean_media' = "`mediadir'/" + `imagevar' ///
        if strpos(`imagevar', "\") == 0 & strpos(`imagevar', "/") == 0
    ...
    display as text "Processing `N' image files..."
    quietly {
        forvalues i = 1/`N' {
            ...
            if fileexists("`old'") {
                ...
                noisily display as error "⚠️ Image file not found for ID `id': `old'"
    ...
    display as text "----------------------------------------"
    display as text "Image Process Complete Summary:"
    display as result "  Successfully copied: `success_count' image file(s)"
    if `fail_count' > 0 display as error "  Failed / Missing:    `fail_count' image file(s)"
    display as text "----------------------------------------"
